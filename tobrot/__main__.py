...
if __name__ == "__main__" :
    ...
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command([Commandi."STATUS@uploadinghb4allbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command([Commandi."SAVETHUMBNAIL@uploadinghb4allbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command([Commandi."CLEARTHUMBNAIL@uploadinghb4allbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
